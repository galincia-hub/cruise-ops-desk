...
# ── CSS ──────────────────────────────────────────────────────────────
CSS = r"""/* ── 상수 ── */
:root{--sidebar-w:272px;--header-h:56px;--radius:12px;--radius-sm:8px;--accent:#10B981;--accent-blue:#3B82F6;--transition:0.2s ease}
:root{--bg-main:#FAFAFA;--bg-header:#FFF;--bg-sidebar:#FFF;--bg-card:#FFF;--bg-table:#FFF;--bg-thead:#F5F5F5;--bg-hover:#EFF6FF;--border-main:#E5E5E5;--border-table:#EBEBEB;--text-primary:#1A1A1A;--text-secondary:#666;--text-nav:#555;--text-nav-head:#333;--nav-active-bg:#F0F0F0;--nav-hover-bg:#F7F7F7;--nav-section-c:#AAA;--nav-dot:#CCC;--nav-main-bg:#F0F1F4;--shadow-card:0 1px 3px rgba(0,0,0,.06),0 4px 16px rgba(0,0,0,.06);--shadow-lg:0 4px 24px rgba(0,0,0,.10);--thead-color:#333}
body.dark{--bg-main:#0F0F1A;--bg-header:#12121F;--bg-sidebar:#1A1A2E;--bg-card:#1E1E30;--bg-table:#1E1E30;--bg-thead:#252540;--bg-hover:#252545;--border-main:#2A2A40;--border-table:#2A2A40;--text-primary:#E0E0E0;--text-secondary:#8888AA;--text-nav:#A0A0C0;--text-nav-head:#C0C0D8;--nav-active-bg:#252545;--nav-hover-bg:#1E1E38;--nav-section-c:#555570;--nav-dot:#404060;--nav-main-bg:#222240;--shadow-card:0 1px 3px rgba(0,0,0,.3),0 4px 16px rgba(0,0,0,.3);--shadow-lg:0 4px 24px rgba(0,0,0,.5);--thead-color:#E0E0E0}

*,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Noto Sans KR',sans-serif;background:var(--bg-main);color:var(--text-primary);line-height:1.65;transition:background var(--transition),color var(--transition)}

.header{position:fixed;top:0;left:0;right:0;height:var(--header-h);z-index:100;background:var(--bg-header);border-bottom:1px solid var(--border-main);display:flex;align-items:center;padding:0 20px;gap:12px;transition:background var(--transition),border-color var(--transition)}
...
.badge-ver{padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;background:var(--nav-active-bg);color:var(--text-secondary)}
.dark-toggle{width:36px;height:36px;border-radius:50%;border:1px solid var(--border-main);background:var(--nav-active-bg);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:16px;transition:all var(--transition)}
.dark-toggle:hover{border-color:var(--accent);transform:scale(1.08)}

.sidebar{position:fixed;top:var(--header-h);left:0;bottom:0;width:var(--sidebar-w);background:var(--bg-sidebar);border-right:1px solid var(--border-main);overflow-y:auto;z-index:90;transition:transform .3s ease,background var(--transition),border-color var(--transition)}
.sidebar::-webkit-scrollbar{width:4px}
.sidebar::-webkit-scrollbar-thumb{background:var(--border-main);border-radius:2px}
.nav-section{padding:14px 0 4px}
.nav-section-title{padding:0 16px;font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:var(--nav-section-c);font-weight:600;margin-bottom:6px}
.nav-item{display:flex;align-items:center;padding:9px 16px;cursor:pointer;color:var(--text-nav);font-size:13px;font-weight:400;border-left:3px solid transparent;transition:background .12s,color .12s,border-color .12s}
.nav-item:hover{background:var(--nav-hover-bg);color:var(--text-nav-head)}
.nav-item.active{background:var(--nav-active-bg);color:var(--accent);border-left-color:var(--accent);font-weight:600}
...
.nav-sub .nav-item::before{content:'';display:inline-block;width:5px;height:5px;border-radius:50%;background:var(--nav-dot);margin-right:10px;flex-shrink:0}
.nav-sub .nav-item.active::before{background:var(--accent)}
.nav-group{border-bottom:1px solid var(--border-main)}
.nav-group-header{display:flex;align-items:center;padding:10px 16px;cursor:pointer;color:var(--text-nav-head);font-size:13px;font-weight:600;background:var(--nav-main-bg);margin:2px 8px;border-radius:6px;transition:background .12s}
...
.stat-card{background:var(--bg-card);border:1px solid var(--border-main);border-radius:var(--radius-sm);padding:16px;text-align:center;box-shadow:var(--shadow-card)}
...
.ms-card{background:var(--bg-card);border:1px solid var(--border-main);border-radius:var(--radius-sm);padding:14px;box-shadow:var(--shadow-card);transition:transform .15s}
...
.ms-dday{border-left:4px solid var(--accent);background:linear-gradient(135deg,var(--bg-card),#ECFDF5)}
body.dark .ms-dday{background:linear-gradient(135deg,var(--bg-card),#064E3B)}

/* 팀 헤더 */
.team-header{background:var(--bg-card);border:1px solid var(--border-main);border-radius:var(--radius-sm);padding:20px;margin-bottom:24px;box-shadow:var(--shadow-card)}
...
.section-heading{font-size:15px;font-weight:700;color:var(--text-primary);margin:24px 0 12px;padding-bottom:8px;border-bottom:2px solid var(--border-main)}

/* 대시보드 */
.dash-section-title{font-size:15px;font-weight:700;color:var(--text-primary);margin:28px 0 12px;padding-bottom:8px;border-bottom:2px solid var(--border-main)}
...
.dash-card{background:var(--bg-card);border:1px solid var(--border-main);border-radius:var(--radius);overflow:hidden;box-shadow:var(--shadow-card);cursor:pointer;transition:box-shadow var(--transition),transform var(--transition)}
...
.sheet-list li{padding:8px 0;border-bottom:1px solid var(--border-main);font-size:13px;display:flex;align-items:center;cursor:pointer;color:var(--text-secondary);transition:color .12s}
...
.table-wrap{overflow-x:auto;margin-bottom:28px;border-radius:var(--radius-sm);border:1px solid var(--border-main);box-shadow:var(--shadow-card);transition:border-color var(--transition)}
.section-title{padding:11px 18px;font-size:13px;font-weight:700;color:#FFF;letter-spacing:.2px}
.ops-table{width:100%;border-collapse:collapse;background:var(--bg-table);font-size:13px;transition:background var(--transition)}
.ops-table thead th{padding:10px 12px;font-size:12px;font-weight:700;text-align:center;letter-spacing:.3px;white-space:nowrap;background:#F5F6F8!important;color:#333!important;border-bottom:2px solid #E0E4EA}
.ops-table tbody tr{background:color-mix(in srgb,var(--row-bg,transparent) 35%,#FFF);min-height:40px}
.ops-table tbody td{padding:10px 12px;font-size:13px;line-height:1.6;vertical-align:top;border-bottom:1px solid #EEF2F7;color:var(--text-primary);text-align:center}
...
.ops-table tbody td.merged-desc{padding:14px 20px;font-size:13px;line-height:1.7;color:var(--text-secondary);background:#F9FAFB;border-bottom:1px solid #EEF2F7}
body.dark .ops-table tbody td.merged-desc{background:#1a1a2e!important;color:#8888AA!important;border-bottom-color:#2A2A40}
body.dark .ops-table thead th{background:#252540!important;color:#D8D8F0!important;border-bottom-color:#2A2A40}
body.dark .ops-table tbody tr{background:#1E1E30!important}
body.dark .ops-table tbody td{color:#D8D8E8!important;border-bottom-color:#2A2A40}
...
/* 조직도 */
.org-chart-section{margin-bottom:28px;padding:24px;background:var(--bg-card);border:1px solid var(--border-main);border-radius:var(--radius);box-shadow:var(--shadow-card)}
.org-chart{display:flex;flex-direction:column;align-items:center;gap:16px}
.org-node{border:2px solid var(--border-main);border-radius:10px;padding:12px 16px;background:var(--bg-card);min-width:220px;transition:all var(--transition)}
...
.org-legend{display:flex;gap:12px;flex-wrap:wrap;justify-content:center;padding:12px;margin-top:8px;border-top:1px solid var(--border-main)}

/* 매트릭스 필터 */
.matrix-toolbar{margin-bottom:16px;padding:14px 16px;background:var(--bg-card);border:1px solid var(--border-main);border-radius:var(--radius-sm)}
.matrix-search-row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}
.matrix-search{flex:1;min-width:200px;padding:8px 12px;border:1px solid var(--border-main);border-radius:6px;font-size:13px;background:var(--bg-main);color:var(--text-primary);outline:none;transition:border-color .2s}
.matrix-search:focus{border-color:var(--accent-blue)}
.matrix-btn{padding:7px 14px;border:1px solid var(--border-main);border-radius:6px;background:var(--bg-card);color:var(--text-primary);font-size:12px;font-weight:600;cursor:pointer;transition:all .15s;white-space:nowrap}
...
window.addEventListener('DOMContentLoaded',function(){var d=document.body.classList.contains('dark');document.getElementById('dark-btn').textContent=d?'☀️':'🌙';if(document.getElementById('matrix-toolbar'))matrixInit()});
